...
class SellPosController extends Controller
{
                ...
                if (!$is_direct_sale) {
                    if ($input['status'] == 'draft') {
                        ...
                        if ($input['is_quotation'] == 1) {
                             //added Qoutation
                            $this->notificationUtil->autoSendNotification($business_id, 'new_quotation', $transaction, $transaction->contact);
    ...
    private function receiptContent(
        ...
        if ($receipt_printer_type == 'printer') {
            ...
            if ($show_invoice){
                $output['html_content'] = view('sale_pos.receipts.classic', compact('receipt_details'))->render();
            }else{
    ...
    public function getProductSuggestion(Request $request)
    {
        if ($request->ajax()) {
            ...
            // Here this brand id is now sub-catgory id coz at front change brand to sub category
    ...
    public function showInvoice($token)
    {
        ...
        if (!empty($transaction)) {
            ...
            $receipt = $this->receiptContent($transaction->business_id, $transaction->location_id, $transaction->id, 'browser', false, false, $invoice_layout_id,true);
