...
<section class="content">
	...
	<div class="row">
        ...
        @if((int)$price_group->id == 1)
        <td>
            {!! Form::text('group_prices[' . $price_group->id . '][' . $variation->id . ']', !empty($variation_prices[$variation->id][$price_group->id]) ? @num_format($variation_prices[$variation->id][$price_group->id]) : 0, ['class' => 'form-control input_number input-sm' , 'id'=>'twenty'] ); !!}
        </td>
        @endif 
       
        @if((int)$price_group->id == 2)
        <td>
            {!! Form::text('group_prices[' . $price_group->id . '][' . $variation->id . ']', !empty($variation_prices[$variation->id][$price_group->id]) ? @num_format($variation_prices[$variation->id][$price_group->id]) : 0, ['class' => 'form-control input_number input-sm' , 'id'=>'thirty']  ); !!}
        </td>
        @endif
...
$("#twenty").val({{$variation->default_purchase_price}} + {{$variation->default_purchase_price}} * 20 /100);

$("#thirty").val({{$variation->default_purchase_price}} + {{$variation->default_purchase_price}} * 30 /100);

$("#five").val({{$variation->default_purchase_price}} + {{$variation->default_purchase_price}} * 5 /100);

$("#ten").val({{$variation->default_purchase_price}} + {{$variation->default_purchase_price}} * 10 /100);
