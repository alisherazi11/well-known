...
<td style="text-align: left;">
	@foreach($price_groups as $k => $v)
		...
		<div class="input-group" style="width: 100%;">
			...
			@if((int)$k == 1)
			{!! Form::text('products[' . $product->id . '][variations][' . $variation->id . '][group_prices][' . $k . ']', !empty($price_grp) ? @num_format($price_grp->price_inc_tax) : 0, ['class' => 'form-control input-sm input_number twenty']); !!}
			@endif 
		
			@if((int)$k == 2)
			{!! Form::text('products[' . $product->id . '][variations][' . $variation->id . '][group_prices][' . $k . ']', !empty($price_grp) ? @num_format($price_grp->price_inc_tax) : 0, ['class' => 'form-control input-sm input_number thirty']); !!}
			@endif
		</div>
	@endforeach
</td>