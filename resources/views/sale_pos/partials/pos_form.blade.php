		...
		<div class="col-md-4 col-sm-6">	
			<div class="form-group">	
				<div class="input-group">	
					...
					{!! Form::select('price_group', $price_groups, $selected_price_group, ['class' => 'form-control select2', 'id' => 'price_group',  'autocomplete' => 'off']); !!}
					<span class="input-group-addon">
						@show_tooltip(__('lang_v1.price_group_help_text'))
					</span>
				</div>
			</div>
			<div class="form-group hidden-field" id="price-password-container">
				<div class="input-group">
					<span class="input-group-addon">
						<i class="fas fa-lock"></i>
					</span>
                    {!! Form::text('price_password', '', ['class' => 'form-control', 'id' => 'price_password', 'autocomplete' => 'off']); !!}
					<span class="input-group-addon" id="price-password-check">@show_tooltip(__('For this option a password is required'))</span>
				</div>
			</div>
		</div>
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                $( '#price_group' ).on('change', function() {
                    ...
                    $( '#price_password' ).attr('required', false);
                    if( !$('#price-password-container').hasClass('hidden-field') ) $('#price-password-container').addClass('hidden-field');
					
					reset_pos_form_price_group();
					if( $(this).val() == '1' ){
						
                        //alert('Your choice is 20%');
                        $( '#price_password' ).attr('required', true);
                        if( $('#price-password-container').hasClass('hidden-field') ) $('#price-password-container').removeClass('hidden-field');
                    }
                });
                //when clicked empty the input
                $( '#price_password' ).on('click', function() {
                    $(this).val('');
                });
                //when input lost focus check what has been typed
                $("#price_password").blur(function(){
                    var val = $.trim($(this).val()); 
                    var md5_pwd = md5( val ); 
                    if(dsoft_required == md5_pwd){
                        $('#price-password-check').html('<i class="fas fa-check-circle" style="color:#006600"></i>');
                    }
                    else{
                        $('#price-password-check').html('<i class="fas fa-times-circle" style="color:#CC0000"></i>');
                        $(this).val('');
                    }
                });
            }, false);
