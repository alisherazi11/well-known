...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
            ...
            <button type="button" class="btn bg-navy btn-default @if(!$is_mobile) @endif btn-flat no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('custom_pay_1', $payment_types)) hide @endif pos-express-finalize @if($is_mobile) col-xs-6 @endif" data-pay_method="custom_pay_1" title="@lang('custom_pay_1')"> <i class="fas fa-money-bill-alt" aria-hidden="true"></i> @lang('lang_v1.express_checkout_shipping')</button>

            <button type="button" class="btn btn-success @if(!$is_mobile) @endif btn-flat no-print"  title="@lang('sale.shipping')" aria-hidden="true" data-toggle="modal" data-target="#posShippingModal" title="Delivery"> <i cclass="fas fa-edit cursor-pointer" aria-hidden="true"></i> @lang('lang_v1.express_checkout_shipping1')</button>
